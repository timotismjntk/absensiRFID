...
export default function Login({route, navigation}) {
  ...
  useEffect(() => {
    if (authSiswa?.status === 'gagal') {
      Alert.alert(
        'Gagal',
        authSiswa?.pesan?.length > 0
          ? authSiswa?.pesan === 'Username atau password wajib diisi'
  ...
  return (
    <SafeAreaView edges={['bottom', 'right', 'left']} style={styles.container}>
      ...
      <LoadingModal
        open={isLoadingLoginSiswa || false}
        close={() => dispatch(clearStateSiswa())}
      />
      <View style={styles.wrapper}>
        ...
        <View style={styles.form}>
          ...
          <View style={styles.checkBoxForgotPasswordContainer}>
            ...
            <Text
              onPress={() => navigation.navigate('LupaPasswordSiswa')}
