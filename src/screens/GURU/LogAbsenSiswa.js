...
export default function LogAbsenSiswa() {
  ...
  const listNamaSiswa = useMemo(() => {
    if (Array.isArray(dataSiswa?.result?.siswas)) {
      return dataSiswa?.result?.siswas?.map(item => item?.nama || '');
    } else {
      return [];
    }
  }, [dataSiswa]);
  ...
  const lihatAbsenSiswa = useCallback(() => {
    const {siswa_id} =
      dataSiswa?.result?.siswas?.find(item => item?.nama === namaSiswa) || {};
    if (siswa_id) {
      const twoDigitMonth = {
        Januari: '01',
        Februari: '02',
  ...
  }, [dataSiswa, namaSiswa, tahun, bulan]);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <FlatList
        data={memoizedData}
        refreshing={isLoadingLogAbsenSiswa || isLoadingDataSiswa}
        ...
        ListHeaderComponent={
          <View style={styles.wrapper}>
            <View style={styles.form}>
              ...
              <RectButton
                enabled={
                  namaSiswa?.length > 0 && bulan?.length > 0 ? true : false
                }
                ...
                style={[
                  ...
                  {
                    backgroundColor:
                      namaSiswa?.length > 0 && bulan?.length > 0
                        ? '#61A2F9'
                        : 'grey',
                  },
                ]}>
