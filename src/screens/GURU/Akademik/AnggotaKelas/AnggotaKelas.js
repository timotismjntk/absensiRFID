...
export default function AnggotaKelas({navigation}) {
  ...
  const memoizedData = useMemo(() => {
    if (anggotaKelas?.result?.anggotakelas?.length > 0) {
  ...
  const WaliKelas = useCallback(() => {
    return (
      <RectButton
        ...
        onPress={() =>
          navigation.navigate('ProfilGuruLain', {
            user_id_walikelas: anggotaKelas?.result?.user_id_walikelas || '',
          })
        }>
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View style={styles.wrapper}>
            ...
            {anggotaKelas?.result?.nama_walikelas?.length > 0 && <WaliKelas />}
          </View>
        }
        ListEmptyComponent={
        ...
        renderItem={({item}) => (
          <RectButton
            onPress={() => navigation.navigate('ProfilSiswa', item)}
            style={styles.item}>
